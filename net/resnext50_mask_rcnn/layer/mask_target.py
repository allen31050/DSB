...
def make_one_mask_target(cfg, mode, input, proposal, truth_box, truth_label, truth_instance):
    ...
    for i in range(num_proposal):
        box = proposal[i,1:5]
        # if not(is_small_box_at_boundary(box, width, height, min_size = cfg.mask_train_min_size)): valid.append(i)
        if not(is_small_box(box, min_size=cfg.mask_train_min_size)): valid.append(i)
