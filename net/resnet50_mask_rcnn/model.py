...
from ..lib.roi_align_pool_tf.module import RoIAlign as Crop
...
class CropRoi(nn.Module):
    ...
    def forward(self, fs, proposals):
        ...
        for l in range(self.num_scales):
            ...
            if len(index) > 0:
                crop = self.crops[l](fs[l], proposals[index].view(-1,7)) # proposal: i, x0, y0, x1, y1, score, label, (scale_level)
                crops.append(crop)
...
class MaskNet(nn.Module):
    ...
    def forward(self, inputs, truth_boxes=None,  truth_labels=None, truth_instances=None ):
        ...
        if len(self.rcnn_proposals)>0:
              ...
              # self.mask_proposals = self.masks
