import os
# os.environ['CUDA_VISIBLE_DEVICES'] = '0'  #'3,2' #'3,2,1,0' # commented on multi-gpu machine
...
def run_train():
    ...
    out_dir  = RESULTS_DIR + '/mask-rcnn-se-resnet50-gray500' # debug
    initial_checkpoint = RESULTS_DIR + '/mask-rcnn-se-resnet50-gray500/checkpoint/00025000_model.pth'
    # initial_checkpoint = RESULTS_DIR + '/mask-se-resnext50-rcnn_2crop-mega-05c/checkpoint/00049000_model.pth'

    pretrain_file = None # RESULTS_DIR + '/mask-rcnn-50-gray500-clean/checkpoint/15021_model.pth' # None
    skip = ['crop','mask']
    ...
    num_iters   = 30 * 1000 # 1000 * 1000 # 29K iterations (30 * 1000)
    iter_smooth = 20
    iter_log    = 50
    iter_valid  = 100
    ...
    LR = None # StepLR([ (0, 0.01), (10000, 0.001), (20000, 0.0001), (25000, 0.00001)])
    optimizer = optim.SGD(filter(lambda p: p.requires_grad, net.parameters()), lr=0.001/iter_accum, momentum=0.9, weight_decay=0.0001)
    ...
    train_dataset = ScienceDataset(
                            # 'train2_ids_color_152', mode='train',
                            # 'train2_ids_all_701', mode='train',
                            'train_and_valid2_ids_all_765', mode='train',

                            # 'train1_ids_gray2_500', mode='train',
                            # 'train2_ids_gray2_549', mode='train',
                            # 'train2_ids_core_25', mode='train',
                            # 'train2_ids_wraped_127', mode='train',
    ...
    valid_dataset = ScienceDataset(
                            # 'valid2_ids_color_14', mode='train',
                            'valid2_ids_all_61', mode='train',

                            # 'valid1_ids_gray2_43', mode='train',
                            # 'valid2_ids_gray2_47', mode='train',
                            # 'valid2_ids_core_3', mode='train',
                            # 'valid2_ids_wraped_11', mode='train',
